...
export class BacktestEngine {
  ...
  private rangeHigh: number | null = null;
  private rangeLow: number | null = null;

  // Tracking
  private balance: number;
  private peak: number;
  private trades: BacktestTradeResult[] = [];
  ...
  run(candles: Candle[]): BacktestResult {
    ...
    const warmupPeriod = Math.max(
      this.config.swingLength * 2 + 1,
      this.config.slowMAPeriod + 1
    );

    if (candles.length < warmupPeriod) {
      return this.buildErrorResult(
        `Insufficient candles: ${candles.length} < ${warmupPeriod}`,
        startTime
      );
    }
  ...
  private checkEntries(analysis: ReturnType<typeof this.analyzeMarket>, candle: Candle): void {
  ...
  private checkTpSl(candle: Candle): boolean {
  ...
  private checkExits(analysis: ReturnType<typeof this.analyzeMarket>, candle: Candle): void {
  ...
  private closePosition(exitPrice: number, exitTime: Date, exitReason: string): void {
    if (!this.position) return;
  ...
  private buildResult(startTime: number, candleCount: number): BacktestResult {
  ...
  private buildErrorResult(error: string, startTime: number): BacktestResult {
    return {
      config: this.config,
      status: "failed",
      errorMessage: error,
